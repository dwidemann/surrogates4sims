...
__all__ = ['preprocessFcn', 'invPreprocessFcn', 'MantaFlowSVDDataset']
...
from .utils import rmse, create_movie, convertSimToImage, pkl_save, pkl_load, rel_err
from tqdm import tqdm
import pickle

#Cell
def preprocessFcn(X,p):
        x_range = 11.953
        X /= x_range
        p_range = [[0.2, 0.8], [0.04, 0.12], [0.0, 199.0]]
        for i, ri in enumerate(p_range):
            p[i] = (p[i]-ri[0]) / (ri[1]-ri[0]) * 2 - 1
        return X,p

def invPreprocessFcn(X,p):
        x_range = 11.953
        X *= x_range
        p_range = [[0.2, 0.8], [0.04, 0.12], [0.0, 199.0]]
        for i, ri in enumerate(p_range):
            p[i] = (p[i]+1.)*(ri[1]-ri[0])/2 + ri[0]
        return X,p
